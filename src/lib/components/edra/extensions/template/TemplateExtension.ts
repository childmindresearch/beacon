...
class TemplateNodeView {
    ...
    private isAtSentenceStart(view: EditorView): boolean {
        ...
        const textBefore = doc
            .textBetween(Math.max(0, pos - lookBackDistance), pos)
            .trim()
...
export const TemplateExtension = Node.create<TemplateOptions>({
    ...
    addAttributes() {
        return {
            name: {
                default: null,
                parseHTML: (el) =>
                    (el as HTMLSpanElement).getAttribute('data-name'),
                renderHTML: (attrs) => ({ 'data-name': attrs.name }),
            },
            options: {
                default: [],
                parseHTML: (el) => {
                    const optionsStr = (el as HTMLSpanElement).getAttribute(
                        'data-options'
                    )
    ...
    addCommands() {
        return {
            setTemplate:
                (variable: string, options: string[], color: string = 'blue') =>
                ({ commands }) => {
